...
from falcon import HEAD_LEN, SALT_LEN, SHAKE256, Falcon, params
from ffsampling import ffldl, ffldl_fft, ffnp, ffnp_fft, gram
from fft import add, div, fft, ifft, mul, neg, sub
from ntrugen import gs_norm, karamul, ntru_gen
from ntt import div_zq, mul_zq
from samplerz import MAX_SIGMA, samplerz
...
def test_compress(n, iterations):
    ...
    try:
        sigma = 1.5 * sqrt(q)
        param = params[n]
        slen = param.sig_bytelen - SALT_LEN - HEAD_LEN
    except KeyError:
        return True
    for i in range(iterations):
        while 1:
...
def test_signature(n, iterations=10):
    ...
    G = sign_KAT[n][0]["G"]
    falcon = Falcon(n)
    sk, vk = falcon.keygen([f, g, F, G])
    for i in range(iterations):
        message = b"abc"
        sig = falcon.sign(sk, message)
        if falcon.verify(vk, message, sig) is False:
            return False
    return True


def test_unpack_and_sign_KAT():
    """
    ...
    for n in sign_KAT:
        ...
        for D in sign_KAT_n:
            ...
            # We also test if packing/unpacking sk is correct: pack and unpack sk,
            # and check that the unpacked sk2 is identical to sk.
            sk_bytes = falcon.pack_sk(sk)
            sk2 = falcon.unpack_sk(sk_bytes)
            if sk != sk2:
                return False
            ...
            sig = falcon.sign(sk, message, shake.read)
...
def wrapper_test(my_test, name, n, iterations):
    ...
    if rep is True:
        ...
        message += f" ({msec} msec / execution)".rjust(30)
    print(message)
...
if __name__ == "__main__":
    ...
    print("OK" if (test_unpack_and_sign_KAT() is True) else "Not OK")
