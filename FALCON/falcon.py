...
from FALCON.ffsampling import ffldl_fft, ffsampling_fft, gram
from FALCON.fft import add_fft, fft, ifft, mul_fft, neg, sub
from FALCON.ntrugen import ntru_gen
from FALCON.ntt import div_zq, mul_zq, sub_zq
...
def deserialize_to_poly(bytestring: bytes, n: int) -> list[int]:
    ...
    poly = [0] * n
    int_buffer = int.from_bytes(bytestring, "little")
    for idx in range(n):
...
class Falcon:
    ...
    def unpack_sk(self, sk_bytes: bytes):
        ...
        for i in range(4):
            polys[i] = deserialize_to_poly(
                sk_bytes[i * len_poly : (i + 1) * len_poly], self.param.n
            )
    ...
    def __sample_preimage__(self, B0_fft, T_fft, point, seed=None):
        ...
        if seed is None:
            ...
            z_fft = ffsampling_fft(
                t_fft, T_fft, self.param.sigmin, chacha_prng.randombytes
            )
    ...
    def verify(self, vk: bytes, message: bytes, signature: bytes) -> bool:
        ...
        if 8 * len(vk) % self.param.n != 0:
            raise ValueError(
                f"This does not seem to be a valid key for Falcon-{self.param.n}"
            )
        h = deserialize_to_poly(vk, self.param.n)
        salt = signature[HEAD_LEN : HEAD_LEN + SALT_LEN]
        enc_s = signature[HEAD_LEN + SALT_LEN :]
        s1 = decompress(
            enc_s, self.param.sig_bytelen - HEAD_LEN - SALT_LEN, self.param.n
        )
