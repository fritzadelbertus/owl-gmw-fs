...
from OWL.coder import decode_poly_matrix, encode_poly_matrix
from OWL.group_action import action, group_inverse, group_operator, regeneratefg
import OWL.hawk.ntrugen
from OWL.hawk.ntrugen.ntrugen_hawk import ntru_solve
from OWL.hawk.params import PARAMS
from OWL.hawk.poly import (
    adjoint,
    infnorm,
    isinvertible,
    l2norm,
...
def owl_Gen(rng=None):
    ...
    # Line 28: return (priv, pub)
    return encode_poly_matrix(
        [
            [[i % p for i in q00], [i % p for i in q01]],
            [[i % p for i in q10], [i % p for i in q11]],
        ]
    ), kgseed
...
def owl_Sign(privateKeyInBits, publicKeyInBits, messageInByte):
    ...
    p = 8380417
    private_key = [
        [[i % p for i in f], [i % p for i in F]],
        [[i % p for i in g], [i % p for i in G]],
    ]
    ...
    h_i = [
        [[i % p for i in hf], [i % p for i in hF]],
        [[i % p for i in hg], [i % p for i in hG]],
    ]
