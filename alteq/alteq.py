...
from ALTEQ.atf import acting_on_atfs, inverting_on_atf
from ALTEQ.expand import (
    expand_atfs,
    expand_challenge,
    expand_columns,
    expand_seeds,
...
from ALTEQ.matrix import columns_decomposition, columns_matrix
from ALTEQ.params import (
    CHLG_SIZE,
    LEN,
    MAT_SK_SEED_SIZE,
    MSG_HASH_SIZE,
...
def alteq_keygen():
    ...
    columns = [
        ...
        for x in expand_columns(
            seeds[r * MAT_SK_SEED_SIZE : (r + 1) * MAT_SK_SEED_SIZE]
        )
    ]
...
def alteq_sign(message: list[bytes], secret_key: bytes):
    if len(message) <= 0:
        raise ValueError("sign: can't sign empty message")
        return None
    ...
    while success == 0:
        ...
        if C >= K:
            temp_columns = [
                ...
                for r in chg_val
                for x in expand_columns(
                    seeds_sk[r * MAT_SK_SEED_SIZE : (r + 1) * MAT_SK_SEED_SIZE]
                )
            ]
        else:
            ...
            for r in range(K):
                if expanded[chg_val[r]] != -1:
                    temp_columns[r * (N * N) : (r + 1) * N * N] = temp_columns[
                        expanded[chg_val[r]] * (N * N) : (expanded[chg_val[r]] + 1)
                        * N
                        * N
                    ]
                else:
                    temp_columns[r * (N * N) : (r + 1) * N * N] = expand_columns(
                        seeds_sk[
                            chg_val[r] * MAT_SK_SEED_SIZE : (chg_val[r] + 1)
                            * MAT_SK_SEED_SIZE
                        ]
                    )
