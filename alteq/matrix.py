from ALTEQ.field import inversion_modulo_p, multiplication_mod_p, reduction_mod_p
from ALTEQ.params import PRIME, K, N
            ...
            for k in range(s, N):
                for r in range(K):
                    mat[(i * N + k) * K + r] = reduction_mod_p(
                        (mat[(i * N + k) * K + r] & 0xFFFFFFFFFFFFFFFF)
    ...
    for k in range(s, N):
        for r in range(K):
            mat[(j * N + k) * K + r] = reduction_mod_p(
                multiplication_mod_p(mat[(j * N + k) * K + r], column[(j) * K + r])
            )
        ...
        if i != j:
            for r in range(K):
                col[(i) * K + r] = reduction_mod_p(
                    multiplication_mod_p((PRIME - col[(j) * K + r]), col[(i) * K + r])
                )
    return col


def columns_matrix(cols_a, cols_b):
    mat = [0] * N * N * K

    for i in range(N):
        for j in range(N):
            for r in range(K):
                mat[(i * N + j) * K + r] = cols_a[(j * N + i) * K + r]

    for j in range(N - 1, -1, -1):
        mat = column_mul(mat, cols_a[j * N * K : (j + 1) * N * K], j, j + 1)
    for j in range(N - 1, -1, -1):
        mat = column_mul(mat, cols_b[j * N * K : (j + 1) * N * K], j, 0)
...
def columns_decomposition(columns, nb_mats):
    ...
    for j in range(1, N):
        ...
        for i in range(N):
            columns[(j * N + i) * K : (j * N + i + 1) * K] = mat0[
                (i * N + j) * K : (i * N + j + 1) * K
            ]
