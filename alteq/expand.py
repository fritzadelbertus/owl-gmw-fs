...
from ALTEQ.params import LEN, PRIME, ROUND, C, K, N
from ALTEQ.random_rng import deterministic_sample, get_random_value, shake_rng
...
def expand_seeds(seed: bytes, n_seeds: int, out_seed_size: int) -> list[bytes]:
    ...
    return [
        expanded[i * out_seed_size : (i + 1) * out_seed_size] for i in range(n_seeds)
    ]
...
def expand_columns(seed: bytes, storage_size: int = 256) -> list[int]:
    ...
    storage = [
        int.from_bytes(raw_bytes[4 * r : 4 * (r + 1)], "little")
        for r in range(storage_size)
    ]
...
def expand_atfs(
    seed: bytes, nb_copies: int, vec_size: int, storage_size: int = 256
) -> list[int]:
    """
