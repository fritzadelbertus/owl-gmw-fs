from ALTEQ.params import LOG_Q, PRIME, C, N


def multiplication_mod_p(a: int, b: int) -> int:
    r = a * b
    return r - PRIME * (r >> LOG_Q)
...
def set_inversion_mod_p(set_list):
    ...
    for i in range(1, N):
        for r in range(C):
            mul[i * C + r] = reduction_mod_p(
                multiplication_mod_p(mul[(i - 1) * C + r], set_list[i * C + r])
            )
    ...
    for i in range(N - 1, 0, -1):
        for r in range(C):
            tmp = reduction_mod_p(multiplication_mod_p(mul[(i - 1) * C + r], inv0[r]))
            inv0[r] = reduction_mod_p(
                multiplication_mod_p(inv0[r], set_list[i * C + r])
            )
            set_list[i * C + r] = tmp
